 ...
 * synchronizers (semaphores, events, etc) that rely on(依靠  基于)
 ...
 * and which define what that state means in terms of this object being
 * acquired or released.(定义就同步对象获取或释放而言状态[state]的意义)
 *
 * Given these(鉴于这些), the other methods in this class carry out(完成)
 * all queuing and blocking mechanics.(排队和阻塞机制)
 *
 * Subclasses can maintain other state fields, but only the atomically updated {@code int}
 * value manipulated using methods {@link #getState}, {@link#setState} and {@link #compareAndSetState}
 ...
 * Instead(然而) it defines methods such as {@link #acquireInterruptibly} that can be invoked as
 * appropriate by(适用于) concrete(具体) locks and related synchronizers to implement their public methods.
 *
 * This class supports either or both(任一或全部) a default <em>exclusive</em> mode and a <em>shared</em> mode.
 *
 ...
 *
 * This class defines a nested {@link java.util.concurrent.locks.AbstractQueuedSynchronizer.ConditionObject} class
 * that can be used as a {@link Condition} implementation by subclasses supporting exclusive mode for
 * which method {@link#isHeldExclusively} reports whether synchronization is exclusively held(保持) with
 * respect to the current thread, method {@link #release} invoked with the current {@link #getState} value fully releases
 * this object, and {@link #acquire}, given this saved state value,eventually(最终) restores(恢复) this object to its previous acquired state.
 *
 * No {@code AbstractQueuedSynchronizer} method otherwise creates such a condition,
 * so if this constraint(约束) cannot be met, do not use it.
 *
 * The behavior of {@link java.util.concurrent.locks.AbstractQueuedSynchronizer.ConditionObject}
 * depends of course on the semantics(语义) of its synchronizer implementation.
 *
 * This class provides inspection(检查), instrumentation(仪表), and monitoring methods
 * for the internal queue,as well as similar(类似) methods for condition objects.
 *
 * These can be exported as desired(想要的) into classes using an {@code AbstractQueuedSynchronizer} for their
 * synchronization mechanics.
 *
 * Serialization(序列化) of this class stores only the underlying(底层的) atomic integer maintaining state,
 * so deserialized objects have empty thread queues.
 *
 * Typical(典型) subclasses requiring serializability will define a {@code readObject} method
 * that restores this to a known initial state upon deserialization.
 *
 * Even though this class is based on an internal FIFO queue, it does not automatically
 * enforce(强制) FIFO acquisition policies.
 *
 * Because checks in acquire are invoked before enqueuing, a newly acquiring thread may
 * <em>barge</em> ahead of(之前) others that are blocked and queued.
 *
 * However, you can, if desired(想要的),define {@code tryAcquire} and/or {@code tryAcquireShared} to
 * disable barging by internally invoking one or more of the inspection methods,
 * thereby(从而) providing a <em>fair</em> FIFO acquisition order.
 *
 * In particular(特定), most fair synchronizers can define {@code tryAcquire} to return {@code false}
 * if {@link #hasQueuedPredecessors} (a method specifically designed to be used by fair synchronizers)
 * returns {@code true}.  Other variations(变化) are possible.
 *
 * Throughput(吞吐量) and scalability(伸缩性) are generally highest for the default barging (also known as <em>greedy(贪婪)</em>,
 * <em>renouncement(放弃)</em>, and <em>convoy-avoidance</em>) strategy.
 *
 * @date 2019-12-01
 */
public abstract class AbstractQueuedSynchronizer
     ...
     * contender(竞争者) thread may need to rewait.
     *
     ...
     * demarcation(划界) from unqueued to queued. Similarly, dequeuing
